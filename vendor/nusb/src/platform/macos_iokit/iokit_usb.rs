...
impl IoKitInterface {
    ...
    pub(crate) fn open(&mut self) -> Result<(), Error> {
        // maschine fork: first try USBInterfaceOpenSeize, which evicts
        // whatever driver is bound. On modern macOS DriverKit-based HID
        // drivers may refuse to yield on the first try — we retry a few
        // times with short sleeps to let the kernel tear down the old
        // binding before reattempting.
        let mut last_rc = io_kit_sys::ret::kIOReturnSuccess;
        for attempt in 0..5 {
            let rc = unsafe { call_iokit_function!(self.raw, USBInterfaceOpenSeize()) };
            if rc == io_kit_sys::ret::kIOReturnSuccess {
                if attempt > 0 {
                    log::info!("USBInterfaceOpenSeize succeeded after {} retries", attempt);
                }
                return Ok(());
            }
            log::debug!("USBInterfaceOpenSeize attempt {}: rc=0x{:08x}", attempt, rc as u32);
            last_rc = rc;
            std::thread::sleep(std::time::Duration::from_millis(75));
        }
        check_iokit_return(last_rc)
    }
