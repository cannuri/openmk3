...
impl MacDevice {
    pub(crate) fn from_device_info(d: &DeviceInfo) -> Result<Arc<MacDevice>, Error> {
        ...
        // maschine fork: USBDeviceOpenSeize evicts existing kernel-side
        // drivers (MIDIServer, usbaudiod, IOHIDFamily) so our client can
        // claim the device. This is the same trick Chrome WebUSB uses on
        // macOS to reach vendor-defined interfaces.
        let opened = match unsafe { call_iokit_function!(device.raw, USBDeviceOpenSeize()) } {
    ...
    fn require_open_exclusive(&self) -> Result<(), Error> {
        ...
        if *state == false {
            // maschine fork: Seize variant — see from_device_info.
            unsafe { check_iokit_return(call_iokit_function!(self.device.raw, USBDeviceOpenSeize()))? };
            *state = true;
        }
